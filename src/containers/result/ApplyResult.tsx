...
function ApplyResult(props: Props) {
  const { code } = props;
  const [data, setData] = useState<TResult>();

  const handleGetResult = async () => {
    const response = await axios
      .get(`${NEXT_PUBLIC_API_URL}/auditions/detail/${code}`)
      .then((res) => {
          ...
          <div className="col-span-4 ">
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <div className="flex flex-col">
                <span className="text-gray-700 uppercase font-bold tracking-wider mb-4 text-xl">
                  Apply for
                </span>
                <ul>
                  {data?.contracts?.map((contract: any) => (
                    <li className="mb-2  flex gap-2" key={contract?.id}>
                      <b className="capitalize">
                        {contract?.entertainment_company?.nickname}:{' '}
                      </b>
                      <div
                        className={`${APPLY_STATUS_COLOR[contract?.apply_status]} px-2 rounded-2xl`}
                      >
